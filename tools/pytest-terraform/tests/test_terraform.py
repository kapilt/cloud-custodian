...
@tf.aws_terraform('local_foo', scope='function')
...
def test_tf_resources():
    ...
    assert str(excinfo.value).splitlines()[
        0] == "Ambigious resource name rest_api"


@pytest.mark.skipif(
    not tf.find_binary('terraform'),
    reason="Terraform binary missing")
def test_tf_runner(testdir, tmpdir):
    # ** requires network access to install plugin **
    factory = tf.TerraformRunner.install_plugins(
        ['local'], testdir.tmpdir.strpath)
...
''')
    ...
    trunner = factory(tmpdir.strpath)
    trunner.init()
    resources = trunner.apply()
...
def xtest_help_message(testdir):
    result = testdir.runpytest(
        '--help',
    )
    # fnmatch_lines does an assertion internally
...
def test_plugins_ini_setting(testdir):
    ...
    testdir.makepyfile("""
        ...
        @pytest.fixture
        def hello(request):
            return request.config.getini('terraform-plugins')
